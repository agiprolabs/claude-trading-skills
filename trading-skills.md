...
Complete 4-layer multi-timeframe system, built and tested one layer at a time. Layer 1: 1h + 30m HMA(20) slope regime read, 5 states (NEUTRAL/EARLY_BULL/BULL/EARLY_SELL/SELL) with no-lookahead cross-timeframe alignment. Layer 2: 1h + 15m dynamic bias scoring on ROC9/RSI14/ADX14/Volume (rolling percentile-rank, not fixed thresholds), with a stricter pass bar for early vs. already-confirmed regimes. Layer 3: regime-locked directional entry via 15m break-of-structure (fractal swing high/low), where going long during a permitted downtrend is structurally impossible. Layer 4: risk management (5% margin/position, 2 positions max portfolio-wide, 1 per symbol, ATR(14)x1.5 stops, split TP1/TP2 at 0.5R/1.2R with the stop moved to breakeven+0.1R after TP1) ties everything into a runnable multi-symbol portfolio backtest.
